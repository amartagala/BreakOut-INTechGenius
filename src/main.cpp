...
Color ballColor = GetColor(0xFFC300FF);
...
const int particleMaxSpeed = 270;
int maxParticles = 30;
...
int main()
{
    ...
    font = LoadFontEx(filename, fontSize, 0, 0);
...
void RenderScore()
{
    ...
    if(isGameStarted && !isGameEnded)
    {
        DrawTextEx(font,TextFormat("%s%i", text, score),{textPosition.x, textPosition.y}, fontSize, fontSpacing, BLUE);
        return;
    }

    BlurEffect(font,TextFormat("%s%i", text, score),{textPosition.x, textPosition.y}, fontSize, fontSpacing, BLUE);
}
...
void RenderHighScore()
{
    ...
    if(isGameStarted && !isGameEnded)
    {
        DrawTextEx(font, TextFormat("%s%i", text, highscore), {textPosition.x, textPosition.y}, fontSize, fontSpacing, GetColor(0xFFC300FF));
        return;
    }

    BlurEffect(font, TextFormat("%s%i", text, highscore), {textPosition.x, textPosition.y}, fontSize, fontSpacing, GetColor(0xFFC300FF));
}
...
{  
    ...
    float restartTextFontSize = 28.0f;
...
void InitializeLifeRects()
{
    ...
    // float posX = ((WIDTH - 100) + ballWidth - ((ballWidth * 3) + 15.0f * (3 - 1))) * 0.5f;
    float posX = WIDTH - 120;
    float posY = HEIGHT - 30;

    for (int i = 0; i < 3; i++)
    {
        Vector2 liveRectPos = {posX, posY};
        LifeRects liveRect = {liveRectPos, GetColor(0xFFC300FF)};
